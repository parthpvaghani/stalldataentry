...
function convertModelObject(gltfmodel) {
  ...
  return melzogltfmodel;
}

function convertMenu3dObject(menu3d) {
  const id = menu3d._id;
  const melzoMenu3d = {};
  melzoMenu3d.id = id;
...
function convertMenu3dCellObject(cell) {
  const cellId = cell._id;
  const melzoCell = {
    id: cellId,
    type: WIDGET_TYPES.menu3dCell,
...
function convertCarouselObject(carousel) {
  const items = {};
  ...
  return melzoCarousel;
}

function convertCarouselItemObject(item, carousel) {
  const carouselItems = {
    id: item._id,
    src: item.src,
    esol: {},
  ...
  if (item.target && item.target !== 'UNKNOWN') {
    ...
    // eso
    const esoId = 'esoCarousel' + item._id;
    carouselItems.esol[esoId] = targetToEso(esoId, item.target);
  }
  return carouselItems;
}
//
function convertGridObject(grid) {
  const cells = {};
...
function convertGridCellObject(cell) {
  const melzoGridCell = {
    id: cell._id,
    src: cell.src,
    rowIndex: cell.rowIndex,
  ...
  if (cell.target && cell.target !== 'UNKNOWN') {
    // TODO: Implement it properly.
    // class="raycaster-solid"
    melzoGridCell.components = {
      class: 'raycaster-solid clickable'
    };

    melzoGridCell.opacity = 0.8;

    // eso
    const esoId = 'esocurvedImg' + cell._id;
    melzoGridCell.esol[esoId] = targetToEso(esoId, cell.target);

    const scaleUpId = 'anim-scaleup';
    melzoGridCell.animations[scaleUpId] = {
      id: scaleUpId,
      name: 'scaleup',
    ...
    const scaleDownId = 'anim-scaledown';
    melzoGridCell.animations[scaleDownId] = {
      id: scaleDownId,
      name: 'scaledown',
  ...
  return melzoGridCell;
}

function convertTextObject(text3d) {
...
function convertVideoObject(video) {
  const melzoVideo = {
    id: video._id,
    type: 'video2d',
    src: video.src,
...
function convertImagePopupObject(imagePopup) {
...
function convertSoundObject(sound) {
...
function convertImageObject(image) {
...
function convertImage360ViewObject(image) {
  const imageId = image._id;
...
function convertSwitchbuttonObject(switchBtn) {
  ...
  return melzoSwitchBtn;
}

// isEnabled: ,
function convertAudioPAObject(audiopa) {
...
export {
  ...
  convertSceneToMelzo,
  convertImageObject,
  convertImage360ViewObject,
  convertMenu3dObject,
  convertSwitchbuttonObject,
  convertImagePopupObject,
  convertAudioPAObject,
  convertVideoObject,
  convertCarouselObject,
  convertGridObject,
  convertTextObject,
  convertSoundObject,
  convertGifObject,
  convertMenu3dCellObject,
  convertModelObject,
  convertGridCellObject,
  convertCarouselItemObject
};
