...
function targetToEso(id, target) {
  if (target) {
    ...
    if (target.includes('melzounite')) {
      return {
        id,
        sourceId: 'self',
        sourceEvent: 'click',
        eventToEmit: 'uniterequest',
        metaData: {
          bubble: true,
          link: target
        }
    ...
    if (target.includes('https://drive.google.com')) {
      return {
        id,
        sourceId: 'self',
        sourceEvent: 'click',
        eventToEmit: 'externallinkrequest',
        metaData: {
          bubble: true,
          link: target
        }
    ...
    if (target.includes('https://wa.me/')) {
      return {
        id,
        sourceId: 'self',
        sourceEvent: 'click',
        eventToEmit: 'whatsapprequest',
        metaData: {
          bubble: true,
          link: target
        }
    ...
    if (target.includes('expo.virtualpropexpo2020.com')) {
      return {
        id,
        sourceId: 'self',
        sourceEvent: 'click',
        eventToEmit: 'postrequest',
        metaData: {
          bubble: true,
          link: target
        }
...
function strToVector(str) {
  ...
  return { x: strArr[0], y: strArr[1], z: strArr[2] };
}
...
function convertCarousel(carousels) {
  ...
  carousels.forEach(carousel => {
    ...
    const melzoCarousel = {
      ...
      geometry: { height: carousel.height / 2, width: carousel.width / 2 },
      material: { opacity: carousel.opacity },
      slideWaitTime: { unit: 'ms', value: 500 },
      animationDuration: { unit: 'ms', value: carousel.interval * 1000 },
      autoPlay: carousel.autoSwitch,
      direction: 'normal',
      slides: { order: 'organized', items },
      persistent: {
  ...
  imagePopups.forEach(imagePopup => {
    ...
    const melzoPopupParent = {
      ...
      transform: {
        ...
        scale: { x: imagePopup.scale, y: imagePopup.scale, z: imagePopup.scale }
      },
      geometry: {
    ...
    if (isGlobalPositionUpdated(imagePopup.popup.globalPosition)) {
      ...
      childPos = {
        x:
          parseFloat(melzoPopupParent.transform.position.x) +
          imagePopup.popup.position.x,
        y:
          parseFloat(melzoPopupParent.transform.position.y) +
          imagePopup.popup.position.y,
        z:
          parseFloat(melzoPopupParent.transform.position.z) +
  ...
  sounds.forEach(sound => {
    ...
    const melzoSound = {
      ...
      transform: {
        ...
        scale: { x: sound.scale, y: sound.scale, z: sound.scale }
      },
      geometry: {
        height: sound.height,
        width: sound.width
...
function convertSwitchbuttons(switchBtns) {
  ...
  switchBtns.forEach(switchBtn => {
    ...
    const melzoSwitchBtn = {
      id: switchBtnId,
      src:
        'https://d2me8xtj4564r4.cloudfront.net/image/public/image2d/switch-button.png',
      type: 'image2d',
      transform: {
        position: strToVector(switchBtn.position),
        scale: { x: switchBtn.scale, y: switchBtn.scale, z: switchBtn.scale }
      },
      material: {
  ...
  audiopas.forEach(audiopa => {
    ...
    const melzoAudioPA = {
      ...
      transform: {
        ...
        scale: { x: audiopa.scale, y: audiopa.scale, z: audiopa.scale }
      },
      geometry: {
...
function convertSceneToMelzo(scene) {
  ...
  melzoScene.scenename = scene.scenename;
...
function translatePptvrToMelzoForViewer(pptvrJSON) {
  const melzoJSON = {};
  ...
  if (pptvrJSON.reticleProperties) {
    ...
    melzoJSON.reticleProperties = { size: 0.01, color: "white" };
  }
  ...
  if (pptvrJSON.isSplashEnabled === 'undefined' || pptvrJSON.isSplashEnabled === null) {
  ...
  if (pptvrJSON.isCenterPosition === 'undefined' || pptvrJSON.isCenterPosition === null) {
    melzoJSON.isCenterPosition = false;
  } else {
  ...
  const maxNumberOfVideo2dInScene = getMaxNumberOfVideo2dInScene(
    melzoJSON.scenes
  );
  const videoToBeAdded=3-maxNumberOfVideo2dInScene;
  if(melzoJSON.scenes){
    const firstSceneId =scenes[0].sceneid;
  for(let i=0;i<videoToBeAdded;i+=1){
    const videoWidget={
      id:("5f87cfcbffd3ea001107d7b6"+i),
      type:'video2d',
      src:'video/public/2sec.mp4',
      autoPlay:false,
      transform:{position:{x:0.64,y:-13.96,z:1.99},scale:{x:1,y:1,z:1}},
      geometry:{height: 1, width: 1, depth: 20},
      opacity:0,
      esol:{},
      persistent:{isEnabled:false,excludeFrom:''},
      volume:0,
      enablePopup:false,
      lock:false
    }
    melzoJSON.scenes[firstSceneId].widgets["5f87cfcbffd3ea001107d7b6"+i]=videoWidget;
  }
}
  return melzoJSON;
}

function getMaxNumberOfVideo2dInScene(scenesObj) {
  const scenes = Object.values(scenesObj);
  let sceneIndex = 0;
  let maxNumberOfVideo2dInScene = 0;
  for (sceneIndex = 0; sceneIndex < scenes.length; sceneIndex += 1) {
    const scene = scenes[sceneIndex];
    const sceneWidgets = Object.values(scene.widgets);
    let videoCount = 0;
    for (let i = 0; i < sceneWidgets.length; i += 1) {
      if (sceneWidgets[i].type === WIDGET_TYPES.video2d) {
        videoCount += 1;
      }
    }
    maxNumberOfVideo2dInScene = Math.max(maxNumberOfVideo2dInScene, videoCount);
  }
  return maxNumberOfVideo2dInScene;
}

export { translatePptvrToMelzoForViewer, convertSceneToMelzo };
